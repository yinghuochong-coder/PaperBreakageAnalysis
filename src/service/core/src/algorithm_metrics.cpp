        ...
        std::stop_token) noexcept override
    {
        ...
        std::vector<Point> points{
            ...
            {.name = "algorithm.skipped_frames_total",
             ...
             .unit = "count",
             .available = available},
            {.name = "algorithm.sampled_skipped_frames_total",
             .value = snapshot.sampled_skipped_frames,
             .unit = "count",
             .available = available},
            {.name = "algorithm.missed_processing_slots_total",
             .value = snapshot.missed_processing_slots,
             .unit = "count",
             .available = available},
            {.name = "algorithm.configured_processing_fps",
             .value = static_cast<std::uint64_t>(snapshot.configured_processing_fps),
             .unit = "frames_per_second",
             .available = available},
            {.name = "algorithm.failures_total",
             ...
             .unit = "count",
             .available = available},
            {.name = "algorithm.rearm_pending_lanes",
             .value = static_cast<std::uint64_t>(snapshot.rearm_pending_lanes),
             .unit = "count",
             .available = available},
            {.name = "algorithm.rearm_suppressed_results_total",
             .value = snapshot.rearm_suppressed_results,
             .unit = "count",
             .available = available},
        ...
        if (runtime)
        {
            for (const auto& lane : runtime->algorithm_snapshots())
            {
                ...
                points.push_back({.name = prefix + ".skipped_frames_total",
                                  ...
                                  .unit = "count",
                                  .available = true});
                points.push_back({.name = prefix + ".sampled_skipped_frames_total",
                                  .value = metrics.sampled_skipped_frames,
                                  .unit = "count",
                                  .available = true});
                points.push_back({.name = prefix + ".missed_processing_slots_total",
                                  .value = metrics.missed_processing_slots,
                                  .unit = "count",
                                  .available = true});
                points.push_back(
                    {.name = prefix + ".configured_processing_fps",
                     .value = static_cast<std::uint64_t>(metrics.configured_processing_fps),
                     .unit = "frames_per_second",
                     .available = true});
                ...
                points.push_back({.name = prefix + ".result_queue_rejected_total",
                                  ...
                                  .unit = "count",
                                  .available = true});
                points.push_back({.name = prefix + ".rearm_pending",
                                  .value = metrics.rearm_pending,
                                  .unit = "boolean",
                                  .available = true});
                points.push_back({.name = prefix + ".rearm_suppressed_results_total",
                                  .value = metrics.rearm_suppressed_results,
                                  .unit = "count",
                                  .available = true});
