...
AlgorithmConfigurationValue configuration_value(const Json& value)
{
    ...
    return {.enabled = value.value("enabled", false),
            ...
            .downsample_mode = value.value("downsampleMode", std::string{"half"}),
            .processing_fps = value.value("processingFps", 15U),
            .candidate_threshold = value.value("candidateThreshold", 0.6),
            .confirmation_threshold = value.value("confirmationThreshold", 0.8),
            .confirmation_duration_ms = value.value("confirmationDurationMs", 120U),
            .cooldown_ms = value.value("cooldownMs", 1000U),
            .rearm_duration_ms = value.value("rearmDurationMs", 500U),
...
Json configuration_json(const AlgorithmConfigurationValue& value)
{
    return {{"enabled", value.enabled},
            ...
            {"downsampleMode", value.downsample_mode},
            {"processingFps", value.processing_fps},
...
AlgorithmRuntimeValue runtime_value(const Json& value)
{
    ...
    return {
        ...
        .metrics = {
            ...
            .sampled_skipped_frames = metrics.value("sampledSkippedFrames", std::uint64_t{}),
            .missed_processing_slots = metrics.value("missedProcessingSlots", std::uint64_t{}),
            .configured_processing_fps = metrics.value("configuredProcessingFps", 0U),
            ...
            .rearm_pending = metrics.value("rearmPending", false),
            .rearm_suppressed_results = metrics.value("rearmSuppressedResults", std::uint64_t{}),
