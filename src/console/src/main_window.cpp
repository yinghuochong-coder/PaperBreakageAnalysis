...
QString utf8_text(const std::string_view value)
{
...
{
  public:
    ResponsiveGrid(const int minimum_item_width, const int maximum_columns, const int spacing = 10)
        : minimum_item_width_(minimum_item_width), maximum_columns_(maximum_columns),
          spacing_(spacing)
    {
        auto layout = std::make_unique<QGridLayout>(this);
        grid_ = layout.get();
        grid_->setContentsMargins(0, 0, 0, 0);
        grid_->setHorizontalSpacing(spacing_);
        grid_->setVerticalSpacing(spacing_);
    ...
    void add_widget(QWidget* widget)
    {
        ...
        relayout(column_count_);
    }

    [[nodiscard]] QSize minimumSizeHint() const override
    {
        QSize hint = QWidget::minimumSizeHint();
        hint.setWidth(minimum_item_width_);
        return hint;
    }
  ...
  protected:
    void resizeEvent(QResizeEvent* event) override
    {
        ...
        update_columns(event->size().width());
    }

  private:
    void update_columns(const int available_width)
    {
        const int columns =
            std::clamp((std::max(0, available_width) + spacing_) / (minimum_item_width_ + spacing_),
                       1, maximum_columns_);
        if (columns == column_count_)
            return;
        relayout(columns);
    }

    void relayout(const int columns)
    {
        column_count_ = std::max(1, columns);
        ...
        for (std::size_t index = 0; index < widgets_.size(); ++index)
        {
            ...
            grid_->addWidget(widgets_[index], position / column_count_, position % column_count_);
        }
        for (int column = 0; column < maximum_columns_; ++column)
            grid_->setColumnStretch(column, column < column_count_ ? 1 : 0);
        setProperty("layoutColumns", column_count_);
        updateGeometry();
    }

    QGridLayout* grid_{};
    std::vector<QWidget*> widgets_;
    int minimum_item_width_{};
...
QWidget* make_compact_field(QWidget* parent, const QString& title, QWidget* editor)
{
    QWidget* field = make_child<QWidget>(parent);
    auto* layout = make_layout<QVBoxLayout>(field);
    layout->setContentsMargins(8, 5, 8, 5);
    layout->setSpacing(4);
    auto* label = make_child<QLabel>(field, title);
    label->setProperty("role", "statusTitle");
...
QPushButton* make_algorithm_metric_card(QWidget* parent,
                                        const AlgorithmMetricDescriptor& descriptor)
{
    auto* card = make_child<QPushButton>(parent);
    card->setObjectName(QStringLiteral("algorithm-metric-card-%1").arg(utf8_text(descriptor.key)));
    card->setCheckable(true);
    card->setMinimumHeight(72);
    card->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Preferred);
    card->setStyleSheet(QStringLiteral("text-align:left; padding:8px;"));
    card->setText(QStringLiteral("%1\n— %2")
                      .arg(utf8_text(descriptor.chinese_name), utf8_text(descriptor.unit)));
    card->setToolTip(QStringLiteral("%1（%2）\n%3")
                         .arg(utf8_text(descriptor.chinese_name), utf8_text(descriptor.key),
                              utf8_text(descriptor.definition)));
    return card;
}

QWidget* make_algorithm_debug_card(QWidget* parent, const QString& title, const QString& unit,
                                   const QString& tooltip, QLabel*& value)
{
    auto* card = make_child<QFrame>(parent);
    card->setFrameShape(QFrame::StyledPanel);
    card->setToolTip(tooltip);
    card->setMinimumHeight(66);
    auto* layout = make_layout<QVBoxLayout>(card);
    layout->setContentsMargins(8, 6, 8, 6);
    layout->setSpacing(3);
    auto* name = make_child<QLabel>(card, title);
    name->setProperty("role", "statusTitle");
    value = make_child<QLabel>(card, QStringLiteral("— %1").arg(unit));
      ...
      storage_actions_(std::move(storage_actions)), uplink_actions_(std::move(uplink_actions))
{
    ...
    status_grid->addWidget(make_status_item(header, QStringLiteral("软件版本"), version_value_), 0,
                           7);
    status_grid->addWidget(make_status_item(header, QStringLiteral("断纸输入"), line_input_value_),
                           0, 8);
    line_input_value_->setObjectName(QStringLiteral("aggregate-line-input-status"));
    ...
    for (std::size_t index = 0; index < descriptors.size(); ++index)
    {
        ...
        if (descriptor.id == ConsolePageId::camera_configuration)
        {
            ...
            scroll->setObjectName(QStringLiteral("camera-configuration-scroll"));
            scroll->setWidgetResizable(true);
            scroll->setHorizontalScrollBarPolicy(Qt::ScrollBarAlwaysOff);
            QWidget* content = make_child<QWidget>(scroll);
            content->setObjectName(QStringLiteral("camera-configuration-content"));
            auto* content_layout = make_layout<QHBoxLayout>(content);
            content_layout->setContentsMargins(2, 2, 8, 2);
            content_layout->setSpacing(14);

            auto* sidebar = make_child<QWidget>(content);
            sidebar->setObjectName(QStringLiteral("camera-device-sidebar"));
            sidebar->setMinimumWidth(280);
            sidebar->setMaximumWidth(330);
            auto* sidebar_layout = make_layout<QVBoxLayout>(sidebar);
            sidebar_layout->setContentsMargins(0, 0, 0, 0);
            sidebar_layout->setSpacing(12);

            auto* configured_group =
                make_child<QGroupBox>(sidebar, QStringLiteral("已配置槽位与状态"));
            configured_group->setObjectName(QStringLiteral("camera-configured-panel"));
            configured_group->setProperty("role", "cameraPanel");
            ...
            camera_configuration_value_->setObjectName(
                QStringLiteral("camera-configuration-status"));
            ...
            sidebar_layout->addWidget(configured_group);

            auto* discovered_group = make_child<QGroupBox>(sidebar, QStringLiteral("网络发现设备"));
            discovered_group->setObjectName(QStringLiteral("camera-discovered-panel"));
            discovered_group->setProperty("role", "cameraPanel");
            auto* discovered_layout = make_layout<QVBoxLayout>(discovered_group);
            auto* discover =
                make_child<QPushButton>(discovered_group, QStringLiteral("重新发现设备"));
            ...
            discovered_layout->addWidget(discover);
            ...
            camera_control_actions_->setObjectName(QStringLiteral("camera-main-panel"));
            auto* main_layout = make_layout<QVBoxLayout>(camera_control_actions_);
            main_layout->setContentsMargins(0, 0, 0, 0);
            main_layout->setSpacing(12);

            auto* control_group =
                make_child<QGroupBox>(camera_control_actions_, QStringLiteral("相机控制"));
            control_group->setObjectName(QStringLiteral("camera-control-panel"));
            control_group->setProperty("role", "cameraPanel");
            auto* control_group_layout = make_layout<QVBoxLayout>(control_group);
            auto* action_grid = make_child<ResponsiveGrid>(control_group, 108, 7, 8);
            action_grid->setObjectName(QStringLiteral("camera-control-grid"));
            const auto add_action = [&](const QString& text, const std::string& command,
                                        const bool confirm) {
                auto* button = make_child<QPushButton>(action_grid, text);
            ...
            camera_read_parameters_button_ =
                make_child<QPushButton>(action_grid, QStringLiteral("读取当前参数"));
            ...
            action_grid->add_widget(camera_read_parameters_button_);
            ...
            auto* editor_grid = make_child<ResponsiveGrid>(camera_control_actions_, 280, 3, 12);
            camera_editor_ = editor_grid;
            camera_editor_->setObjectName(QStringLiteral("camera-editor"));
            auto* acquisition_panel =
                make_child<QGroupBox>(camera_editor_, QStringLiteral("采集参数"));
            acquisition_panel->setObjectName(QStringLiteral("camera-acquisition-panel"));
            acquisition_panel->setProperty("role", "cameraPanel");
            auto* acquisition_form = make_layout<QFormLayout>(acquisition_panel);
            acquisition_form->setFieldGrowthPolicy(QFormLayout::AllNonFixedFieldsGrow);
            ...
            auto* roi_panel =
                make_child<QGroupBox>(camera_editor_, QStringLiteral("ROI / 图像方向"));
            roi_panel->setObjectName(QStringLiteral("camera-roi-panel"));
            roi_panel->setProperty("role", "cameraPanel");
            auto* roi_form = make_layout<QFormLayout>(roi_panel);
            ...
            camera_reverse_y_->setObjectName(QStringLiteral("camera-reverse-y"));
            roi_form->addRow(QStringLiteral("宽度"), camera_roi_width_);
            roi_form->addRow(QStringLiteral("高度"), camera_roi_height_);
            roi_form->addRow(QStringLiteral("水平偏移"), camera_roi_x_);
            roi_form->addRow(QStringLiteral("垂直偏移"), camera_roi_y_);
            ...
            transport_panel->setProperty("role", "cameraPanel");
            auto* transport_form = make_layout<QFormLayout>(transport_panel);
            transport_form->setFieldGrowthPolicy(QFormLayout::AllNonFixedFieldsGrow);
            transport_form->setLabelAlignment(Qt::AlignLeft | Qt::AlignVCenter);
            camera_packet_size_ = make_child<QSpinBox>(transport_panel);
            camera_packet_size_->setRange(576, 9000);
            camera_packet_delay_ = make_child<QSpinBox>(transport_panel);
            ...
            camera_operation_value_->setProperty("role", "muted");
            auto* save = make_child<QPushButton>(footer, QStringLiteral("保存并下发"));
            ...
            QObject::connect(save, &QPushButton::clicked, this, [this] {
                ...
                CameraParameterValue value{
                    ...
                    .line_io = {.alarm_input_enabled = camera_alarm_input_enabled_->isChecked(),
                                .alarm_active_level =
                                    camera_alarm_active_level_->currentText().toStdString(),
                                .strobe_output_enabled = camera_strobe_output_enabled_->isChecked(),
                                .strobe_duration_us =
                                    static_cast<std::uint32_t>(camera_strobe_duration_->value()),
                                .strobe_pre_delay_us =
                                    static_cast<std::uint32_t>(camera_strobe_pre_delay_->value()),
                                .strobe_post_delay_us = static_cast<std::uint32_t>(
                                    camera_strobe_post_delay_->value())}};
            ...
            QObject::connect(camera_selector_, &QComboBox::currentIndexChanged, this, [this] {
                update_camera_configuration_summary();
        ...
        if (descriptor.id == ConsolePageId::algorithm_configuration)
        {
            ...
            auto* layout = make_layout<QVBoxLayout>(page);
            layout->setContentsMargins(24, 20, 24, 20);
            auto* heading = make_child<QLabel>(page, QStringLiteral("算法配置与运行指标"));
            heading->setProperty("role", "pageTitle");
            layout->addWidget(heading);
            ...
            auto* editor_layout = make_layout<QVBoxLayout>(algorithm_editor_);
            editor_layout->setContentsMargins(0, 0, 0, 0);
            editor_layout->setSpacing(10);

            auto* basic_group =
                make_child<QGroupBox>(algorithm_editor_, QStringLiteral("基础与执行"));
            auto* basic_layout = make_layout<QVBoxLayout>(basic_group);
            auto* basic_grid = make_child<ResponsiveGrid>(basic_group, 255, 4, 8);
            basic_grid->setObjectName(QStringLiteral("algorithm-config-basic-grid"));
            basic_layout->addWidget(basic_grid);
            algorithm_enabled_ = make_child<QCheckBox>(basic_grid, QStringLiteral("启用自动检测"));
            algorithm_type_ = make_child<QComboBox>(basic_grid);
            algorithm_type_->setObjectName(QStringLiteral("algorithm-type"));
            algorithm_type_->addItem(QStringLiteral("模拟检测器"), QStringLiteral("mock"));
            algorithm_type_->addItem(QStringLiteral("传统视觉"),
                                     QStringLiteral("classical-vision"));
            algorithm_downsample_mode_ = make_child<QComboBox>(basic_grid);
            ...
            algorithm_processing_fps_ = make_child<QComboBox>(basic_grid);
            for (const auto fps : {15, 30, 60})
                algorithm_processing_fps_->addItem(QString::number(fps), fps);
            algorithm_debug_overlay_ =
                make_child<QCheckBox>(basic_grid, QStringLiteral("启用调试可视化数据"));
            basic_grid->add_widget(
                make_compact_field(basic_grid, QStringLiteral("算法启用"), algorithm_enabled_));
            basic_grid->add_widget(
                make_compact_field(basic_grid, QStringLiteral("检测器类型"), algorithm_type_));
            basic_grid->add_widget(make_compact_field(basic_grid, QStringLiteral("算法降采样"),
                                                      algorithm_downsample_mode_));
            basic_grid->add_widget(make_compact_field(basic_grid, QStringLiteral("算法处理 FPS"),
                                                      algorithm_processing_fps_));
            basic_grid->add_widget(make_compact_field(basic_grid, QStringLiteral("调试数据"),
                                                      algorithm_debug_overlay_));
            editor_layout->addWidget(basic_group);

            auto* roi_group = make_child<QGroupBox>(algorithm_editor_, QStringLiteral("检测区域"));
            auto* roi_layout = make_layout<QVBoxLayout>(roi_group);
            auto* roi_grid = make_child<ResponsiveGrid>(roi_group, 255, 4, 8);
            roi_grid->setObjectName(QStringLiteral("algorithm-config-roi-grid"));
            roi_layout->addWidget(roi_grid);
            algorithm_full_frame_ = make_child<QCheckBox>(roi_grid, QStringLiteral("使用完整画面"));
            algorithm_roi_width_ = make_child<QSpinBox>(roi_grid);
            algorithm_roi_height_ = make_child<QSpinBox>(roi_grid);
            algorithm_roi_x_ = make_child<QSpinBox>(roi_grid);
            algorithm_roi_y_ = make_child<QSpinBox>(roi_grid);
            ...
            roi_grid->add_widget(
                make_compact_field(roi_grid, QStringLiteral("ROI 模式"), algorithm_full_frame_));
            roi_grid->add_widget(
                make_compact_field(roi_grid, QStringLiteral("ROI 宽"), algorithm_roi_width_));
            roi_grid->add_widget(
                make_compact_field(roi_grid, QStringLiteral("ROI 高"), algorithm_roi_height_));
            roi_grid->add_widget(
                make_compact_field(roi_grid, QStringLiteral("ROI X"), algorithm_roi_x_));
            roi_grid->add_widget(
                make_compact_field(roi_grid, QStringLiteral("ROI Y"), algorithm_roi_y_));
            editor_layout->addWidget(roi_group);

            auto* decision_group =
                make_child<QGroupBox>(algorithm_editor_, QStringLiteral("判定与模型"));
            auto* decision_layout = make_layout<QVBoxLayout>(decision_group);
            auto* decision_grid = make_child<ResponsiveGrid>(decision_group, 255, 4, 8);
            decision_grid->setObjectName(QStringLiteral("algorithm-config-decision-grid"));
            decision_layout->addWidget(decision_grid);
            algorithm_candidate_threshold_ = make_child<QDoubleSpinBox>(decision_grid);
            algorithm_confirmation_threshold_ = make_child<QDoubleSpinBox>(decision_grid);
            for (auto* threshold :
            ...
            algorithm_confirmation_duration_ms_ = make_child<QSpinBox>(decision_grid);
            ...
            algorithm_device_ = make_child<QComboBox>(decision_grid);
            ...
            decision_grid->add_widget(make_compact_field(decision_grid, QStringLiteral("候选阈值"),
                                                         algorithm_candidate_threshold_));
            decision_grid->add_widget(make_compact_field(decision_grid, QStringLiteral("确认阈值"),
                                                         algorithm_confirmation_threshold_));
            decision_grid->add_widget(make_compact_field(decision_grid,
                                                         QStringLiteral("确认持续时间 (ms)"),
            ...
            decision_grid->add_widget(make_compact_field(
                decision_grid, QStringLiteral("配置模型版本"), algorithm_model_version_));
            decision_grid->add_widget(
                make_compact_field(decision_grid, QStringLiteral("运行设备"), algorithm_device_));
            editor_layout->addWidget(decision_group);
            ...
            auto* export_csv = make_child<QPushButton>(actions, QStringLiteral("导出当前值 CSV"));
            export_csv->setObjectName(QStringLiteral("algorithm-export-current-csv"));
            ...
            auto* metric_group = make_child<QGroupBox>(content, QStringLiteral("运行指标"));
            metric_group->setObjectName(QStringLiteral("algorithm-runtime-metric-groups"));
            ...
            for (const auto group :
                 {AlgorithmMetricGroup::processing_throughput,
                  ...
                  AlgorithmMetricGroup::backlog_stability, AlgorithmMetricGroup::detection_results})
            {
                auto* section = make_child<QGroupBox>(
                    metric_group, utf8_text(algorithm_metric_group_name(group)));
                section->setObjectName(
                    QStringLiteral("algorithm-runtime-group-%1").arg(static_cast<int>(group)));
                auto* section_layout = make_layout<QVBoxLayout>(section);
                auto* grid = make_child<ResponsiveGrid>(section, 230, 4, 8);
                grid->setObjectName(
                    QStringLiteral("algorithm-runtime-grid-%1").arg(static_cast<int>(group)));
                section_layout->addWidget(grid);
                for (const auto& metric_definition : algorithm_metric_descriptors())
                {
                    if (metric_definition.group != group)
                        continue;
                    auto* card = make_algorithm_metric_card(grid, metric_definition);
                    algorithm_metric_cards_.emplace(std::string{metric_definition.key}, card);
                    grid->add_widget(card);
                    QObject::connect(card, &QPushButton::clicked, this,
                                     [this, key = std::string{metric_definition.key}] {
                                         algorithm_selected_metric_ = key;
                                         for (const auto& [card_key, metric_card] :
                                              algorithm_metric_cards_)
                                             metric_card->setChecked(card_key == key);
                                         update_algorithm_metric_chart();
                                     });
                }
                metric_layout->addWidget(section);
            }
            if (const auto selected = algorithm_metric_cards_.find(algorithm_selected_metric_);
                selected != algorithm_metric_cards_.end())
                selected->second->setChecked(true);
            auto* chart_group = make_child<QGroupBox>(metric_group, QStringLiteral("共享曲线"));
            auto* chart_layout = make_layout<QVBoxLayout>(chart_group);
            algorithm_metric_chart_ = make_child<AlgorithmMetricChart>(chart_group);
            algorithm_metric_chart_->setObjectName(QStringLiteral("algorithm-metric-chart"));
            chart_layout->addWidget(algorithm_metric_chart_);
            metric_layout->addWidget(chart_group);
            ...
            auto* debug_groups = make_child<QWidget>(test_group);
            debug_groups->setObjectName(QStringLiteral("algorithm-debug-metric-groups"));
            auto* debug_groups_layout = make_layout<QVBoxLayout>(debug_groups);
            debug_groups_layout->setContentsMargins(0, 0, 0, 0);
            for (const auto group : {AlgorithmDebugMetricGroup::image_detection,
                                     AlgorithmDebugMetricGroup::thresholds_background,
                                     AlgorithmDebugMetricGroup::plugin})
            {
                auto* section = make_child<QGroupBox>(
                    debug_groups, utf8_text(algorithm_debug_metric_group_name(group)));
                section->setObjectName(
                    QStringLiteral("algorithm-debug-group-%1").arg(static_cast<int>(group)));
                auto* section_layout = make_layout<QVBoxLayout>(section);
                auto* grid = make_child<ResponsiveGrid>(section, 230, 4, 8);
                grid->setObjectName(
                    QStringLiteral("algorithm-debug-grid-%1").arg(static_cast<int>(group)));
                section_layout->addWidget(grid);
                if (group == AlgorithmDebugMetricGroup::plugin)
                    algorithm_plugin_debug_grid_ = grid;
                else
                {
                    for (const auto& metric_definition : algorithm_debug_metric_descriptors())
                    {
                        if (metric_definition.group != group)
                            continue;
                        QLabel* value{};
                        auto* card = make_algorithm_debug_card(
                            grid, utf8_text(metric_definition.chinese_name),
                            utf8_text(metric_definition.unit),
                            QStringLiteral("%1（%2）\n%3")
                                .arg(utf8_text(metric_definition.chinese_name),
                                     utf8_text(metric_definition.key),
                                     utf8_text(metric_definition.definition)),
                            value);
                        card->setObjectName(QStringLiteral("algorithm-debug-card-%1")
                                                .arg(utf8_text(metric_definition.key)));
                        algorithm_debug_metric_values_.emplace(std::string{metric_definition.key},
                                                               value);
                        grid->add_widget(card);
                    }
                }
                debug_groups_layout->addWidget(section);
            }
            test_layout->addWidget(algorithm_test_result_);
            test_layout->addWidget(algorithm_test_preview_);
            test_layout->addWidget(debug_groups);
            ...
            QObject::connect(export_csv, &QPushButton::clicked, this,
                             [this] { request_algorithm_csv_export(); });
            pages_->addWidget(page);
            continue;
        }
...
void MainWindow::update_line_input_indicators()
{
    const auto set_indicator = [](QLabel* label, const QString& text, const char* color) {
        if (!label)
            return;
        label->setText(text);
        label->setStyleSheet(
            QStringLiteral("font-weight: 700; color: %1;").arg(QString::fromLatin1(color)));
    };
    switch (aggregate_line_input_state(camera_snapshot_.cameras))
    {
    case CameraLineInputAggregateState::all_disabled:
        set_indicator(line_input_value_, QStringLiteral("● 全部禁用"), "#8a949e");
        break;
    case CameraLineInputAggregateState::active:
        set_indicator(line_input_value_, QStringLiteral("● 有效"), "#e03131");
        break;
    case CameraLineInputAggregateState::active_stale:
        set_indicator(line_input_value_, QStringLiteral("● 有效（已过期）"), "#e03131");
        break;
    case CameraLineInputAggregateState::partially_unknown:
        set_indicator(line_input_value_, QStringLiteral("● 部分未知"), "#d99000");
        break;
    case CameraLineInputAggregateState::all_known_inactive:
        set_indicator(line_input_value_, QStringLiteral("● 无效"), "#2b8a3e");
        break;
    }

    if (!camera_selector_ || camera_selector_->currentIndex() < 0 || !camera_line0_raw_ ||
        !camera_line0_alarm_)
        return;
    const auto index = static_cast<std::size_t>(camera_selector_->currentIndex());
    if (index >= camera_snapshot_.cameras.size())
        return;
    const auto& camera = camera_snapshot_.cameras[index];
    if (!camera.saved.line_io.alarm_input_enabled)
    {
        set_indicator(camera_line0_raw_, QStringLiteral("● 已禁用"), "#8a949e");
        set_indicator(camera_line0_alarm_, QStringLiteral("● 已禁用"), "#8a949e");
    }
    else if (!camera.line_input)
    {
        set_indicator(camera_line0_raw_, QStringLiteral("● 未知"), "#d99000");
        set_indicator(camera_line0_alarm_, QStringLiteral("● 未知"), "#d99000");
    }
    else
    {
        const QString stale = camera.line_input->stale ? QStringLiteral("（已过期）") : QString{};
        set_indicator(
            camera_line0_raw_,
            QStringLiteral("● %1%2").arg(
                camera.line_input->raw_level ? QStringLiteral("高") : QStringLiteral("低"), stale),
            camera.line_input->stale ? "#d99000" : "#2f6fdd");
        set_indicator(
            camera_line0_alarm_,
            QStringLiteral("● %1%2").arg(camera.line_input->alarm_active ? QStringLiteral("有效")
                                                                         : QStringLiteral("无效"),
                                         stale),
            camera.line_input->alarm_active ? "#e03131"
                                            : (camera.line_input->stale ? "#d99000" : "#2b8a3e"));
    }
}

void MainWindow::update_camera_configuration_summary()
{
    if (!camera_configuration_value_ || !camera_selector_)
        return;

    const int selected_index = camera_selector_->currentIndex();
    QString summary;
    if (selected_index < 0 ||
...
void MainWindow::configure_camera_roi_editor(const CameraClientItem& camera)
{
    ...
    const auto configure_strobe = [](QSpinBox* input,
                                     const std::optional<CameraIntegerRangeValue>& range) {
        if (!range)
            return;
        const auto bounded = [](const std::uint32_t value) {
            return static_cast<int>(std::min<std::uint32_t>(
                value, static_cast<std::uint32_t>(std::numeric_limits<int>::max())));
        };
        input->setRange(bounded(range->minimum), bounded(range->maximum));
        input->setSingleStep(std::max(1, bounded(range->increment)));
        input->setToolTip(QStringLiteral("设备实际范围：%1～%2；步进：%3")
                              .arg(input->minimum())
                              .arg(input->maximum())
                              .arg(input->singleStep()));
    };
    if (camera.line_io_capabilities)
    {
        configure_strobe(camera_strobe_duration_, camera.line_io_capabilities->strobe_duration_us);
        configure_strobe(camera_strobe_pre_delay_,
                         camera.line_io_capabilities->strobe_pre_delay_us);
        configure_strobe(camera_strobe_post_delay_,
                         camera.line_io_capabilities->strobe_post_delay_us);
    }
...
void MainWindow::populate_camera_editor(const CameraParameterValue& value)
{
    ...
    camera_alarm_active_level_->setCurrentText(
        QString::fromStdString(value.line_io.alarm_active_level));
    camera_strobe_output_enabled_->setChecked(value.line_io.strobe_output_enabled);
    camera_strobe_duration_->setValue(static_cast<int>(value.line_io.strobe_duration_us));
    camera_strobe_pre_delay_->setValue(static_cast<int>(value.line_io.strobe_pre_delay_us));
    camera_strobe_post_delay_->setValue(static_cast<int>(value.line_io.strobe_post_delay_us));

    const auto selected = camera_selector_ && camera_selector_->currentIndex() >= 0
                              ? static_cast<std::size_t>(camera_selector_->currentIndex())
                              : camera_snapshot_.cameras.size();
    if (selected < camera_snapshot_.cameras.size())
    {
        const auto& actual_parameters = camera_snapshot_.cameras[selected].actual;
...
void MainWindow::update_camera_controls()
{
    ...
    bool alarm_supported = false;
    bool strobe_supported = false;
    QString unsupported_reason = QStringLiteral("连接相机后读取 Line I/O 能力");
    if (configured)
    {
        const auto index = static_cast<std::size_t>(camera_selector_->currentIndex());
        if (index < camera_snapshot_.cameras.size() &&
            camera_snapshot_.cameras[index].line_io_capabilities)
        {
            const auto& capability = *camera_snapshot_.cameras[index].line_io_capabilities;
            alarm_supported = capability.alarm_input_supported &&
                              capability.rising_edge_supported && capability.falling_edge_supported;
            strobe_supported = capability.strobe_output_supported;
            if (!capability.unsupported_reason.empty())
                unsupported_reason = QString::fromStdString(capability.unsupported_reason);
        }
    }
    camera_alarm_input_enabled_->setEnabled(configured && alarm_supported);
    camera_alarm_active_level_->setEnabled(configured && alarm_supported);
    camera_strobe_output_enabled_->setEnabled(configured && strobe_supported);
    camera_strobe_duration_->setEnabled(configured && strobe_supported);
    camera_strobe_pre_delay_->setEnabled(configured && strobe_supported);
    camera_strobe_post_delay_->setEnabled(configured && strobe_supported);
    const QString alarm_reason =
        alarm_supported
            ? QString{}
            : (unsupported_reason.isEmpty() ? QStringLiteral("设备不支持 Line 0 双边沿输入")
                                            : unsupported_reason);
    const QString strobe_reason =
        strobe_supported ? QString{}
                         : (unsupported_reason.isEmpty() ? QStringLiteral("设备不支持 Line 1 频闪")
                                                         : unsupported_reason);
    camera_alarm_input_enabled_->setToolTip(alarm_reason);
    camera_alarm_active_level_->setToolTip(alarm_reason);
    for (auto* widget : {static_cast<QWidget*>(camera_strobe_output_enabled_),
                         static_cast<QWidget*>(camera_strobe_duration_),
                         static_cast<QWidget*>(camera_strobe_pre_delay_),
                         static_cast<QWidget*>(camera_strobe_post_delay_)})
        if (!strobe_supported)
            widget->setToolTip(strobe_reason);
...
void MainWindow::apply_algorithm_snapshot(const AlgorithmClientSnapshot& snapshot)
{
    ...
    static_cast<void>(algorithm_metric_history_.ingest(snapshot));
    ...
    if (!algorithm_editor_ || !algorithm_runtime_status_ || !algorithm_metric_chart_)
        return;
        ...
        algorithm_runtime_status_->setText(
            QStringLiteral("%1\n实际状态：%2 · 相机 %3 · 生效修订 %4（保存修订 %5）\n"
                ...
                .arg(QString::fromStdString(runtime.plugin_id),
                     ...
                     QString::fromStdString(snapshot.effective_configuration.device),
    ...
    for (const auto& descriptor : algorithm_metric_descriptors())
    {
        const auto card = algorithm_metric_cards_.find(std::string{descriptor.key});
        if (card == algorithm_metric_cards_.end())
            continue;
        const auto scalar = algorithm_metric_value(descriptor, metrics);
        QString value;
        if (std::holds_alternative<bool>(scalar))
            value = std::get<bool>(scalar) ? QStringLiteral("是") : QStringLiteral("否");
        else if (std::holds_alternative<double>(scalar))
            value = QString::number(std::get<double>(scalar), 'f', 2);
        else
            value = QString::fromStdString(algorithm_metric_value_text(scalar));
        card->second->setText(
            QStringLiteral("%1\n%2 %3")
                .arg(utf8_text(descriptor.chinese_name), value, utf8_text(descriptor.unit)));
    }
    update_algorithm_metric_chart();

    for (const auto& descriptor : algorithm_debug_metric_descriptors())
    {
        const auto value = algorithm_debug_metric_values_.find(std::string{descriptor.key});
        if (value != algorithm_debug_metric_values_.end())
            value->second->setText(QStringLiteral("— %1").arg(utf8_text(descriptor.unit)));
    }
    if (snapshot.test_result)
    {
        ...
        for (const auto& metric : result.debug_metrics)
        {
            ...
            auto value = algorithm_debug_metric_values_.find(metric.name);
            if (value == algorithm_debug_metric_values_.end() && algorithm_plugin_debug_grid_)
            {
                auto* grid = dynamic_cast<ResponsiveGrid*>(algorithm_plugin_debug_grid_);
                if (grid)
                {
                    QLabel* label{};
                    auto* card = make_algorithm_debug_card(
                        grid, QString::fromStdString(metric.name), QStringLiteral("插件值"),
                        QStringLiteral(
                            "%1\n检测器插件返回的单帧调试值；口径、范围和重置语义由该插件定义。")
                            .arg(QString::fromStdString(metric.name)),
                        label);
                    card->setObjectName(QStringLiteral("algorithm-debug-card-plugin-%1")
                                            .arg(QString::fromStdString(metric.name)));
                    grid->add_widget(card);
                    value = algorithm_debug_metric_values_.emplace(metric.name, label).first;
                }
            }
            if (value != algorithm_debug_metric_values_.end())
                value->second->setText(
                    QStringLiteral("%1 %2")
                        .arg(metric.value, 0, 'g', 10)
                        .arg(descriptor ? utf8_text(descriptor->unit) : QStringLiteral("插件值")));
        }
    }
    else
    {
...
void MainWindow::update_algorithm_metric_chart()
{
    if (!algorithm_metric_chart_)
        return;
    const auto descriptors = algorithm_metric_descriptors();
    const auto descriptor =
        std::ranges::find(descriptors, algorithm_selected_metric_, &AlgorithmMetricDescriptor::key);
    if (descriptor == descriptors.end())
        return;
    const auto history =
        algorithm_metric_history_.history(algorithm_snapshot_.camera_id, descriptor->key);
    std::vector<AlgorithmChartPoint> points;
    points.reserve(history.size());
    for (const auto& point : history)
    {
        const auto milliseconds = std::chrono::duration_cast<std::chrono::milliseconds>(
                                      point.sample_time.time_since_epoch())
                                      .count();
        points.push_back(
            {.sample_time = QDateTime::fromMSecsSinceEpoch(milliseconds), .value = point.value});
    }
    const auto current = algorithm_metric_value(*descriptor, algorithm_snapshot_.runtime.metrics);
    algorithm_metric_chart_->set_series(utf8_text(descriptor->chinese_name),
                                        utf8_text(descriptor->unit), std::move(points),
                                        std::holds_alternative<bool>(current));
}

void MainWindow::request_algorithm_csv_export()
{
    const QString default_name = QStringLiteral("PaperBreakEdge-algorithm-%1-latest.csv")
                                     .arg(QString::fromStdString(algorithm_snapshot_.camera_id));
    const QString destination = QFileDialog::getSaveFileName(
        this, QStringLiteral("导出算法当前值"), default_name, QStringLiteral("CSV 文件 (*.csv)"));
    if (destination.isEmpty())
        return;
    if (!algorithm_actions_.export_current_values)
    {
        show_algorithm_result(Result<void>::failure(
            make_error("IPC_NOT_CONNECTED", Severity::warning, "算法客户端尚未初始化", "console",
                       "console.algorithm.exportCurrentValues", true)));
        return;
    }
    const auto result =
        algorithm_actions_.export_current_values(std::filesystem::path{destination.toStdWString()});
    show_algorithm_result(result);
    if (result && algorithm_operation_status_)
        algorithm_operation_status_->setText(QStringLiteral("当前值已导出：%1").arg(destination));
}
