                          ...
                          const std::initializer_list<std::string_view> fields)
{
    ...
    for (const auto& [name, unused] : value.items())
    {
        ...
        {
            return Result<void>::failure(invalid_config(
                "配置包含未声明字段", "config.validateSchema",
                                 ...
                                 const bool allow_empty = false)
{
    ...
    if (!value.is_string())
    {
        return Result<std::string>::failure(invalid_config(
            "配置字段必须是字符串", "config.validateType",
    ...
    {
        return Result<std::string>::failure(invalid_config(
            "配置字符串为空或超过长度上限", "config.validateRange",
                        ...
                        const std::string_view pointer)
{
    ...
    if (!value.is_boolean())
    {
        return Result<bool>::failure(invalid_config(
            "配置字段必须是布尔值", "config.validateType",
            std::string{pointer} + "/" + std::string{name}, "boolean-required"));
    }
                         ...
                         const std::uint64_t maximum)
{
    ...
    {
        return Result<T>::failure(invalid_config(
            "配置字段必须是无符号整数", "config.validateType",
            std::string{pointer} + "/" + std::string{name}, "unsigned-integer-required"));
    }
    ...
    {
        return Result<T>::failure(invalid_config(
            "配置整数超出允许范围", "config.validateRange",
                            ...
                            const double maximum)
{
    ...
    if (!value.is_number())
    {
        return Result<double>::failure(invalid_config(
            "配置字段必须是数值", "config.validateType",
    ...
    {
        return Result<double>::failure(invalid_config(
            "配置数值超出允许范围", "config.validateRange",
            std::string{pointer} + "/" + std::string{name}, "number-range"));
    }
...
Result<RoiConfig> parse_roi(const Json& value, const std::string_view pointer)
{
    ...
    {
        return Result<RoiConfig>::failure(invalid_config(
            "ROI 偏移与尺寸超过安全上限", "config.validateDependency", std::string{pointer},
            "roi-extent"));
    }
    return Result<RoiConfig>::success(
        {.width = width.value(), .height = height.value(), .offset_x = x.value(),
         .offset_y = y.value()});
}
                                  ...
                                  const std::filesystem::path& config_directory)
{
    ...
    {
        return Result<std::string>::failure(invalid_config(
            "配置路径包含禁止的设备路径或空字符", "config.validatePath",
...
bool contains_sensitive_key(const Json& value)
{
    ...
    {
        return std::ranges::any_of(value, [](const Json& child) { return contains_sensitive_key(child); });
    }
    return false;
}
                        ...
                        const std::initializer_list<std::pair<std::string_view, Enum>> choices)
{
    ...
    return Result<Enum>::failure(invalid_config("配置枚举值不受支持", "config.validateEnum",
                                                pointer, "unsupported-enum"));
}
...
{
    return {{"width", roi.width}, {"height", roi.height}, {"offsetX", roi.offset_x},
                                ...
                                const std::filesystem::path& config_directory) noexcept
{
    try
    {
        ...
        if (root.is_discarded())
        {
            return Result<EdgeConfig>::failure(invalid_config(
                "配置不是合法 UTF-8 JSON", "config.parseJson", "", "invalid-json"));
        }
        if (contains_sensitive_key(root))
        {
            return Result<EdgeConfig>::failure(invalid_config(
                "普通配置不得包含密码、Token、Secret 或私钥字段", "config.validateSecrets",
                "", "plaintext-secret-field"));
        }
        if (auto fields = exact_fields(
                root, "", {"configSchemaVersion", "configRevision", "modifiedAt", "system",
                           "cameras", "acquisition", "preview", "algorithm", "event",
                           "storage", "uplink", "plantIo", "logging", "health"});
            !fields)
        {
        ...
        {
            Error error = make_error("SYS_CONFIG_SCHEMA_UNSUPPORTED", Severity::error,
                                     "不支持该配置 schema 版本", "config",
        ...
        {
            return Result<EdgeConfig>::failure(invalid_config(
                "modifiedAt 必须是 UTC RFC 3339 毫秒时间", "config.validateTimestamp",
                "/modifiedAt", "invalid-timestamp"));
        }
        ...
        if (auto fields = exact_fields(system, "/system",
                                       {"machineId", "productionLineId", "shutdownTimeoutMs"});
            !fields)
            return Result<EdgeConfig>::failure(fields.error());
        auto machine = string_field(system, "machineId", "/system", 64U);
        auto line = string_field(system, "productionLineId", "/system", 64U);
        auto shutdown = unsigned_field<std::uint32_t>(system, "shutdownTimeoutMs", "/system",
                                                      1000U, 300000U);
        if (!machine)
        ...
        {
            return Result<EdgeConfig>::failure(invalid_config(
                "设备和生产线 ID 只能包含 ASCII 字母、数字、点、下划线和连字符",
        ...
        {
            return Result<EdgeConfig>::failure(invalid_config(
                "cameras 必须是最多四项的数组", "config.validateSchema", "/cameras",
                "camera-count"));
        }
        ...
        for (std::size_t index = 0; index < cameras.size(); ++index)
        {
            ...
            auto delay = unsigned_field<std::uint32_t>(camera, "triggerDelayUs", pointer, 0U,
                                                       60000000U);
            auto packet = unsigned_field<std::uint32_t>(camera, "packetSizeBytes", pointer, 576U,
                                                        9000U);
            auto inter_packet = unsigned_field<std::uint32_t>(
                camera, "interPacketDelayNs", pointer, 0U, 1000000000U);
            if (!id)
            ...
            {
                return Result<EdgeConfig>::failure(invalid_config(
                    "相机 ID 必须是唯一的 CAM01 至 CAM04", "config.validateDependency",
            ...
            {
                return Result<EdgeConfig>::failure(invalid_config(
                    "启用相机必须配置唯一序列号", "config.validateDependency",
                    pointer + "/serialNumber", "missing-or-duplicate-serial"));
            }
            auto pixel = enum_value<PixelFormat>(
                pixel_text.value(), pointer + "/pixelFormat",
                {{"Mono8", PixelFormat::mono8}, {"Mono10", PixelFormat::mono10},
                 {"Mono12", PixelFormat::mono12}, {"BayerRG8", PixelFormat::bayer_rg8}});
            auto mode = enum_value<TriggerMode>(
                mode_text.value(), pointer + "/triggerMode",
                {{"Continuous", TriggerMode::continuous}, {"Hardware", TriggerMode::hardware},
            ...
            {
                return Result<EdgeConfig>::failure(invalid_config(
                    "硬件触发必须指定 triggerSource", "config.validateDependency",
        ...
        if (auto fields = exact_fields(acquisition, "/acquisition",
                                       {"framePoolCapacity", "queueCapacity",
                                        "receiveTimeoutMs", "threadPriority"});
            !fields)
            return Result<EdgeConfig>::failure(fields.error());
        auto frame_pool = unsigned_field<std::uint32_t>(acquisition, "framePoolCapacity",
                                                        "/acquisition", 1U, 4096U);
        auto queue = unsigned_field<std::uint32_t>(acquisition, "queueCapacity", "/acquisition",
                                                   1U, 4096U);
        auto timeout = unsigned_field<std::uint32_t>(acquisition, "receiveTimeoutMs",
        ...
        if (priority.value() != "low" && priority.value() != "normal" && priority.value() != "high")
            return Result<EdgeConfig>::failure(invalid_config(
                "threadPriority 必须为 low、normal 或 high", "config.validateEnum",
                "/acquisition/threadPriority", "unsupported-enum"));
        if (frame_pool.value() < queue.value())
            return Result<EdgeConfig>::failure(invalid_config(
                "framePoolCapacity 不能小于 queueCapacity", "config.validateDependency",
        ...
        auto preview_width = unsigned_field<std::uint32_t>(preview, "maxWidth", "/preview", 64U,
                                                           8192U);
        auto preview_height = unsigned_field<std::uint32_t>(preview, "maxHeight", "/preview", 64U,
                                                            8192U);
        ...
        result.preview = {.enabled = preview_enabled.value(), .fps = preview_fps.value(),
                          .max_width = preview_width.value(), .max_height = preview_height.value(),
                          .jpeg_quality = jpeg.value()};

        const Json& algorithm = root.at("algorithm");
        if (auto fields = exact_fields(
                algorithm, "/algorithm",
                {"enabled", "type", "roi", "candidateThreshold", "confirmationThreshold",
                 "consecutiveFrames", "cooldownMs", "modelReference", "modelVersion", "device",
                 "debugOverlay"});
            !fields)
            return Result<EdgeConfig>::failure(fields.error());
        ...
        auto candidate_threshold = finite_field(algorithm, "candidateThreshold", "/algorithm", 0.0,
                                                1.0);
        auto confirmation_threshold = finite_field(algorithm, "confirmationThreshold", "/algorithm",
                                                   0.0, 1.0);
        auto consecutive = unsigned_field<std::uint32_t>(algorithm, "consecutiveFrames",
                                                         "/algorithm", 1U, 1000U);
        auto cooldown = unsigned_field<std::uint32_t>(algorithm, "cooldownMs", "/algorithm", 0U,
                                                      3600000U);
        ...
        if (confirmation_threshold.value() < candidate_threshold.value())
            return Result<EdgeConfig>::failure(invalid_config(
                "确认阈值不能低于候选阈值", "config.validateDependency", "/algorithm",
        ...
        if (auto fields = exact_fields(
                event, "/event",
                {"preEventSeconds", "postEventSeconds", "maxEventSeconds", "mergeGapSeconds",
                 "keyFrameCount", "saveRaw", "generatePreviewVideo", "uploadPolicy",
                 "retentionDays"});
            !fields)
            return Result<EdgeConfig>::failure(fields.error());
        ...
        auto maximum = unsigned_field<std::uint32_t>(event, "maxEventSeconds", "/event", 1U,
                                                     3600U);
        ...
        auto retention = unsigned_field<std::uint32_t>(event, "retentionDays", "/event", 1U,
                                                       3650U);
        if (!pre)
        ...
        if (pre.value() + post.value() > maximum.value() || merge.value() > maximum.value())
            return Result<EdgeConfig>::failure(invalid_config(
                "事件前后窗口或合并间隔超过最大事件时长", "config.validateDependency",
                "/event", "event-window"));
        if (upload_policy.value() != "never" && upload_policy.value() != "confirmed" &&
            upload_policy.value() != "all")
            return Result<EdgeConfig>::failure(invalid_config(
                "uploadPolicy 必须为 never、confirmed 或 all", "config.validateEnum",
        ...
        auto warning = unsigned_field<std::uint32_t>(storage, "warningFreeSpaceGiB", "/storage", 1U,
                                                     1000000U);
        auto critical = unsigned_field<std::uint32_t>(storage, "criticalFreeSpaceGiB", "/storage",
                                                      1U, 1000000U);
        auto stop = unsigned_field<std::uint32_t>(storage, "stopFreeSpaceGiB", "/storage", 1U,
                                                  1000000U);
        auto maximum_storage = unsigned_field<std::uint32_t>(
            storage, "maximumEventStorageGiB", "/storage", 1U, 1000000U);
        if (!event_root)
        ...
        if (!(warning.value() > critical.value() && critical.value() > stop.value()))
            return Result<EdgeConfig>::failure(invalid_config(
                "存储水位必须满足 warning > critical > stop", "config.validateDependency",
                "/storage", "storage-watermarks"));
        if (path_from_utf8(event_root.value()).lexically_normal() ==
            path_from_utf8(cache_root.value()).lexically_normal())
            return Result<EdgeConfig>::failure(invalid_config(
                "事件目录和缓存目录不能相同", "config.validateDependency", "/storage",
        ...
        auto heartbeat = unsigned_field<std::uint32_t>(uplink, "heartbeatSeconds", "/uplink", 1U,
                                                       3600U);
        ...
        if (uplink_enabled.value() &&
            ...
            return Result<EdgeConfig>::failure(invalid_config(
                "启用 uplink 时必须使用 HTTPS 和 credentialReference",
        ...
        if (auto fields = exact_fields(plant, "/plantIo",
                                       {"enabled", "adapterType", "endpoint",
                                        "credentialReference", "pollIntervalMs"});
            !fields)
            return Result<EdgeConfig>::failure(fields.error());
        ...
        auto poll = unsigned_field<std::uint32_t>(plant, "pollIntervalMs", "/plantIo", 10U,
                                                  60000U);
        if (!plant_enabled)
        ...
        if (adapter.value() != "disabled" && adapter.value() != "mock")
            return Result<EdgeConfig>::failure(invalid_config(
                "M1 只允许 disabled 或 mock Plant IO 适配器", "config.validateEnum",
                "/plantIo/adapterType", "unapproved-plant-io-adapter"));
        if (plant_enabled.value() && adapter.value() != "mock")
            return Result<EdgeConfig>::failure(invalid_config(
                "启用 Plant IO 时 M1 只允许 mock", "config.validateDependency", "/plantIo",
        ...
        auto log_retention = unsigned_field<std::uint32_t>(logging, "retentionDays", "/logging",
                                                           1U, 3650U);
        auto max_file = unsigned_field<std::uint32_t>(logging, "maximumFileSizeMiB", "/logging", 1U,
                                                      1024U);
        auto max_files = unsigned_field<std::uint32_t>(logging, "maximumFilesPerDay", "/logging",
                                                       1U, 100U);
        auto log_queue = unsigned_field<std::uint32_t>(logging, "queueCapacity", "/logging", 128U,
                                                       65536U);
        if (!level_text)
        ...
        auto level = enum_value<LogLevel>(
            level_text.value(), "/logging/level",
        ...
        if (auto fields = exact_fields(
            ...
            !fields)
            return Result<EdgeConfig>::failure(fields.error());
        auto sample = unsigned_field<std::uint32_t>(health, "sampleIntervalMs", "/health", 100U,
                                                    60000U);
        ...
        auto stale = unsigned_field<std::uint32_t>(health, "heartbeatStaleSeconds", "/health", 1U,
                                                   3600U);
        if (!sample)
    ...
    {
        return Result<EdgeConfig>::failure(invalid_config(
            "配置解析发生未预期错误", "config.parse", "", "unexpected-exception"));
    }
    catch (...)
    {
...
std::string serialize_config(const EdgeConfig& config)
{
    ...
    Json root = {
        ...
        {"system", {{"machineId", config.system.machine_id},
                    {"productionLineId", config.system.production_line_id},
                    {"shutdownTimeoutMs", config.system.shutdown_timeout_ms}}},
        {"cameras", std::move(cameras)},
        {"acquisition", {{"framePoolCapacity", config.acquisition.frame_pool_capacity},
        ...
        {"preview", {{"enabled", config.preview.enabled}, {"fps", config.preview.fps},
                     {"maxWidth", config.preview.max_width},
                     {"maxHeight", config.preview.max_height},
                     {"jpegQuality", config.preview.jpeg_quality}}},
        {"algorithm", {{"enabled", config.algorithm.enabled}, {"type", config.algorithm.type},
        ...
        {"event", {{"preEventSeconds", config.event.pre_event_seconds},
        ...
        {"storage", {{"eventRoot", config.storage.event_root},
        ...
        {"uplink", {{"enabled", config.uplink.enabled}, {"serverUrl", config.uplink.server_url},
        ...
        {"plantIo", {{"enabled", config.plant_io.enabled},
        ...
        {"logging", {{"level", log_level_name(config.logging.level)},
        ...
        {"health", {{"sampleIntervalMs", config.health.sample_interval_ms},
                                              ...
                                              const std::size_t maximum_bytes) noexcept
{
    if (path.empty() || maximum_bytes == 0U)
    {
        return Result<BasicConfigInfo>::failure(invalid_config(
            "配置路径为空或读取上限无效", "config.validate", "", "invalid-input"));
    }
    try
    {
        ...
        if (!stream || !stream.read(contents.data(), static_cast<std::streamsize>(contents.size())))
            return Result<BasicConfigInfo>::failure(invalid_config(
                "配置文件读取不完整", "config.readFile", "", "short-read"));
                                              ...
                                              const EdgeConfig& candidate)
{
    ...
    {
        const bool topology_changed = current.cameras.size() != candidate.cameras.size() ||
            !std::equal(current.cameras.begin(), current.cameras.end(), candidate.cameras.begin(),
                        candidate.cameras.end(), [](const CameraConfig& left,
                                                    const CameraConfig& right) {
