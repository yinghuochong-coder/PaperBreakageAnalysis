...
## 2026-08-12 服务启动自动采集补充

### 目的与范围

- 在 `acquisition` 配置中加入服务启动自动采集开关、失败重试间隔和重试次数；旧配置缺省迁移为不自动采集、间隔 1000 ms、重试 3 次。
- 服务启动时仅处理 `enabled=true` 的已配置槽位，按“连接、应用已保存参数、开始采集”顺序启动；单槽失败独立重试，耗尽后记录错误并降级保留 IPC 运维能力，不阻止其他槽位或服务启动。
- 日志异步工作线程同时输出同一条格式化记录到控制台和按线程滚动文件；不改变采集回调约束、不新增队列或后台线程。
- 范围外：运行中断线恢复策略重构、相机配置 UI、实体相机验证、后续里程碑和无关重构。

### 设计与错误语义

- `acquisition.autoStart`、`startupRetryIntervalMs`、`startupRetryCount` 都属于 `/acquisition` 待重启配置。`startupRetryCount` 表示首次失败后的额外尝试次数，因此默认最多执行 4 次。
- 自动启动编排位于 `paperbreak_service_core`，依赖厂商无关的 `CameraControlRuntime` 和强类型配置；MVS 调用仍只存在于 Hikrobot 适配器。
- 重试等待使用启动 `stop_token` 可取消；关闭时只停止并断开由自动启动组件管理的槽位。单槽最终失败记录稳定业务码、原始 native code（若有）、槽位、阶段和尝试次数。
- 控制台 sink 与文件 sink 共用现有有界异步队列；控制台写入发生在日志工作线程，不进入相机采集回调。

### 实施与测试

- [x] 扩展配置模型、解析/缺省迁移、序列化、v4 schema、默认配置和配置文档，并补充边界/回归测试。
- [x] 新增可测试的相机自动启动生命周期组件，覆盖开关关闭、成功、失败重试、槽位隔离和停止取消。
- [x] 装配服务启动/关闭顺序并记录每次尝试和最终结果。
- [x] 增加控制台日志 sink 与捕获测试，保留文件和 recent tail 行为。
- [x] 只执行静态文本/差异检查；按用户要求不运行 CMake、编译或 CTest，并在证据中明确记录。

### 补充进度与证据

- 2026-08-12：已阅读需求、系统架构、路线图、配置/相机/服务/日志实现和本 ExecPlan；补充任务状态 `in-progress`，工作区开始时无已跟踪修改。
- 2026-08-12：用户明确要求“只编码不编译”，因此本补充不会声称构建、自动化测试或实体相机验证通过。
- 2026-08-12：完成配置、自动启动生命周期、服务装配、控制台日志 sink、Mock 单元测试和文档编码；`default-config.json` 使用 `autoStart=true`、1000 ms、3 次重试。JSON 解析、clang-format 静态格式检查和 `git diff --check` 通过；CMake、编译、CTest 和实体相机均未执行。

### 补充验证证据

| 日期 | 命令/场景 | 结果 | 证据或限制 |
| --- | --- | --- | --- |
| 2026-08-12 | `ConvertFrom-Json` 解析默认配置和 v4 schema | 通过 | 仅语法解析，不替代程序配置校验 |
| 2026-08-12 | clang-format 及 `git diff --check` | 通过 | 仅格式和空白检查 |
| 2026-08-12 | CMake、编译、CTest | 未执行 | 用户明确要求只编码不编译；新增测试已编码但未运行 |
| 2026-08-12 | Hikrobot 实体相机自动连接、参数应用和取流 | 未执行 | 未操作硬件，不声明真机行为通过 |
