...
- 当前状态：BasicDemo 已只读确认一台 `MV-CS020-60GM` 存在；冲突复核时项目探测因两个 `192.168.11.0/24` 接口重复发现同一序列号而拒绝绑定。PLCSIM 随后被外部置为禁用，本任务未修改网卡；最终项目 probe 退出码为 0，发现一台 `MV-CS020-60GM`（`192.168.11.117`，主机接口 `192.168.11.102`）。PLCSIM 永久网段调整、至少一台目标机功能和四路目标硬件/测试交换机验证均未执行；不会擅自改写网卡、执行物理断链或服务控制。
...
| 2026-08-01 | 一台目标机功能、四台实体相机、物理断链、服务重启 | 未执行 | PLCSIM 网段尚未调整，功能/物理动作仍需人工和显式计划 |
...
已实现 Windows/MSVC 真机验证器、MVS 软件触发、版本化有界计划、不可覆盖的原子 JSON 记录、1→4 路固定资源采集和资源指标；新增操作说明、计划样例、人工场景模板和本机清点记录。OFF/ON Debug/Release、全部非硬件 CTest、格式、静态分析及负向门禁通过。BasicDemo 已只读确认一台目标真机存在；项目 probe 在 Realtek 与 PLCSIM 同网段时因同一序列号经两个接口重复发现而按设计拒绝。PLCSIM 被外部禁用后，最终项目 probe 退出码为 0，确认一台目标真机且当前绑定无歧义；批准的 PLCSIM `192.168.12.0/24` 永久调整仍未执行。没有打开设备、写参数、取流或执行物理动作。至少一台目标机功能、四路吞吐/恢复、服务重启与真实预览/日志并发仍未执行，M3-05/M3 保持硬件门禁未完成。
