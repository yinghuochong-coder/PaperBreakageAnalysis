...
提供 Windows/MSVC 可构建的 `PaperBreakCameraHardwareTest`，使用厂商无关相机接口执行只读清点，以及经显式计划文件授权的绑定、参数回读、软触发、单路的限时采集与资源采样；每次运行写出带 schema 版本、UTC 时间、环境、步骤状态、阈值和原始指标的 JSON 记录。提供不擅自执行物理动作的操作手册与记录模板，明确相机网线、交换机链路和服务重启的人工边界。无硬件时以 Mock/伪 SDK 验证工具逻辑和适配层，不把 M3-05 或 M3 标成完成。
...
- 当前状态：PLCSIM 地址已由外部配置迁移到 `192.168.12.222/24`，真机测试窗口内网卡为禁用状态；只有 Realtek `192.168.11.102/24` 持有相机网段。项目唯一发现并绑定一台 `MV-CS020-60GM`（序列号 `DB1888674`、相机 `192.168.11.117`），连续取流、参数回读、软件触发和慢消费者隔离均通过。四路目标硬件、测试交换机、物理断链、服务控制和 M4 真实预览仍未执行。
...
- [x] 至少一台目标相机完成功能验证；
- [ ] 目标吞吐、恢复与预览/日志隔离在生产等价环境留有通过证据；
- [x] 按路线图原始退出条件汇总自动化与至少一台真机功能证据，解除 M3-05/M3 门禁；四路生产等价验证不虚构完成。
...
- 2026-08-01：BasicDemo 只读发现一台 `MV-CS020-60GM`；路由复核确认 Realtek `192.168.11.102/24` 和 Siemens PLCSIM `192.168.11.222/24` 同时拥有相机网段直连路由，重复序列号来自同一设备的双接口发现。PLCSIM 随后被外部置为禁用，本任务没有执行该变更；最终项目 probe 退出码为 0，发现一台 `MV-CS020-60GM`（`192.168.11.117`，主机接口 `192.168.11.102`，独占访问可用）。批准的 PLCSIM `192.168.12.0/24` 永久调整仍待执行。
- 2026-08-01：完成双网卡冲突结论、目标拓扑、操作边界和复测证据的文档修正；未修改枚举代码或系统网络配置。重新执行 ON Debug 构建及非硬件 CTest，22/22 通过；`git diff --check` 通过。
- 2026-08-01：使用 `DB1888674` 执行 60 秒连续真机验证，通过参数回读与 57 fps 门槛；实际 58.35 fps、3545 帧，0 帧隙/超时/不完整帧/池耗尽。
- 2026-08-01：首次软件触发真机验证因阻塞取帧持锁而在触发前连续超时；将软件触发周期下沉到采集线程，新增可取消等待自动化。修复后 20 秒收到 21 帧、0.984 fps、0 超时，验证通过。
- 2026-08-01：20 秒慢消费者真机验证实际 58.34 fps，容量 16 的队列丢最旧 841 帧，0 帧隙/超时/池耗尽，证明不反压采集。按路线图原始退出条件完成 M3-05/M3；四路与生产场景留待 M5/M9 前验证。
...
- DEC-004：M3 门禁严格采用路线图原始条件：自动化适配层通过且至少一台目标相机完成功能验证。四路生产环境、服务重启和 M4 真实预览不能形成 M3 对后续里程碑的循环依赖，但仍是 M5/M9 前必须补齐的生产证据。
- DEC-005：同一序列号经多个主机接口重复发现时保持库存校验失败，不在适配器或工具中静默去重；通过隔离网段消除路由歧义。相机/Realtek 保持 `192.168.11.0/24`，PLCSIM 调整到 `192.168.12.0/24` 且不设置默认网关。

## 意外发现

- 本机 `MVCAM_COMMON_RUNENV` 指向 Development 根，Runtime 位于 `C:\Program Files (x86)\Common Files\MVS\Runtime\Win64_x64`；二者需要分别注入。
- `192.168.11.222` 属于 Siemens PLCSIM 虚拟网卡；它与 Realtek `192.168.11.102/24` 同时生成 `192.168.11.0/24` 直连路由和默认路由，导致 MVS 从两个接口重复发现同一台相机。
...
- probe 的重复判断使用完整、边界校验后的序列号字符串；错误详情调用脱敏函数只保留后四位 `8674`。重复条目现已确认是同一实体设备经 Realtek 和 PLCSIM 两个同网段接口可见；库存校验拒绝是预期安全行为。
...
| 2026-08-01 | `Get-NetAdapter`/`Get-NetIPAddress`/`Get-NetRoute` 只读清点 | 发现冲突 | Realtek `192.168.11.102/24` 与 PLCSIM `192.168.11.222/24` 均有相机网段直连和默认路由；Realtek 物理链路 1Gbps |
| 2026-08-01 | BasicDemo “查找设备” | 发现一台目标真机 | 显示 `MV-CS020-60GM`；只读发现，不代表项目绑定或功能通过 |
| 2026-08-01 | 早期清点阶段的一台目标机功能、四台实体相机、物理断链、服务重启 | 当时未执行 | 后续已完成一台目标机功能；四路与物理场景仍未执行 |
| 2026-08-01 | `PaperBreakCameraHardwareTest --probe --output out/m3-05-probe-20260801.json` | 未通过清单校验，退出 4 | 同一真机经两个同网段接口返回相同序列号；仅披露后四位；未打开设备或取流 |
| 2026-08-01 | PLCSIM 禁用后的 `PaperBreakCameraHardwareTest --probe` | 通过只读发现，退出 0 | 一台 `MV-CS020-60GM`，相机 `192.168.11.117`、接口 `192.168.11.102`、独占访问可用；网卡状态变更非本任务执行，人工场景仍未执行 |
...
| 2026-08-01 | 单相机连续模式 60 秒 | 通过 | 58.35 fps、3545 帧；参数完整回读一致，0 帧隙/超时/不完整帧/池耗尽 |
| 2026-08-01 | 单相机软件触发首次运行 | 未通过并定位缺陷 | 阻塞取帧持锁导致触发命令饥饿；0 帧、3 次连续超时，未误记为通过 |
| 2026-08-01 | 软件触发修复后 20 秒复测 | 通过 | 21 帧、0.984 fps，0 帧隙/超时/不完整帧；触发等待可取消 |
| 2026-08-01 | 单相机慢消费者 20 秒 | 通过 | 58.34 fps，队列容量 16、丢最旧 841 帧，0 帧隙/超时/池耗尽 |

## 完成摘要

已实现 Windows/MSVC 真机验证器、MVS 软件触发、版本化有界计划、不可覆盖的原子 JSON 记录、1→4 路固定资源采集和资源指标。项目在隔离拓扑下唯一发现并绑定 `MV-CS020-60GM / DB1888674`；连续 60 秒、软件触发修复后 20 秒以及慢消费者 20 秒真机验证全部通过。软件触发的真实锁竞争缺陷已修复为采集线程内按可取消周期执行“触发→取帧”，并新增单元测试。ON Debug 完整构建、CTest 22/22 和通用单元入口 149 项通过。自动化与至少一台目标相机功能证据满足路线图 M3 退出条件，M3-05/M3 完成。四台实体相机吞吐、物理断链/交换机恢复、服务重启与 M4 真实预览/日志并发未执行，保留为 M5/M9 前生产等价验证。
