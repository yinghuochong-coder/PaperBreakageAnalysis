...
* 对未完成事件执行恢复或标记损坏；
* 不允许上位机读取仍在写入的事件目录。
* 既有及 D2 升级前的新事件使用 manifest v3 / `PBNVME2` 普通缓冲写；D2 升级后新事件使用
  manifest v4，需要逐帧严格时间信息的原始块使用独立 `PBNVME3`，布局见 ADR-019；两者写入时
  计算一次 SHA-256，不强制刷新且提交/索引不读回负载；
...
* manifest v2/v3 与 `PBNVME1/2` 只读兼容，不迁移、不重写；未来高版本明确拒绝。
