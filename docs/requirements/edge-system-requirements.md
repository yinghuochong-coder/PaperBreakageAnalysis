...
生产采集界面保留曝光时长设置，不提供触发/同步参数编辑，也不提供软件触发操作。Hikrobot
适配器每次开始取流前必须将 `ExposureAuto` 设为 `Off`，并使用配置且经回读确认的
`ExposureTime` 固定曝光；关闭自动曝光失败时不得开始取流。
已有像素格式和触发字段仅保留为配置、IPC 和硬件验证兼容面，本次不修改其 schema。
...
* 顶部四路聚合“断纸输入”灯：任一路有效为红色、全部已知无效为绿色、全部禁用为灰色、部分未知且无有效为黄色；有效状态断线后保持红色并标记已过期。
