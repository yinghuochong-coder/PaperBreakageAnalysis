...
完整端点、信封、限制、命令能力和二进制预览格式见 `docs/uplink-protocol-v1.md`。该安全方案不防窃听、伪造命令或中间人攻击，只能部署在隔离 VLAN，并由防火墙、ACL 和物理访问控制限制来源。`PaperBreakUplinkSimulator` 是 M8-00 参考服务端和人工联调工具，不是完整上位机；边缘端接入仍由 M8-01～M8-04 实现。
