...
上位机通过能力协商后的 `event.lockByUtc` 命令广播同一事件。该命令是机器间实时协同命令，不要求人工二次确认或强制审计；其他远程变更命令继续遵循 Uplink v1 的 `operatorConfirmed` 规则。
...
| 六相机远程预览与慢消费者 | EDGE-PRV-001～004 | 最新帧覆盖、公平轮转、5 fps/2 MiB 上限、断连不反压采集 | 六路真实图像负载 |
...
只有需求对应的代码、模拟测试、协议测试、构建和非硬件 CTest 均通过后，才能声明软件实现完成。PTP 精度、六台实体 MV-CS020-60GM、生产网络、目标 NVMe、真实上位机和长稳测试必须分别记录；未执行时必须明确标记“待硬件/系统联调”，不得推断为通过。
