...
网络断开只延迟上传。可重试失败使用带抖动、有最大间隔和最大次数的指数退避；永久失败与需人工处理不再自动领取，`event.retryUpload` 可把指定事件的失败任务显式放回队列。进程启动把遗留 `InProgress` 原子转为立即到期的 `RetryWait`，保留原任务 ID、事件 ID 和 checkpoint。幂等键至少包含设备 ID、事件 ID、文件逻辑 ID；分块编号和正式校验由 M8-04 扩展。
