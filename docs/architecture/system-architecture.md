...
结果稳定顺序键为单调时间、相机 ID、序号。事件线程以所有 Lane 的自动槽、人工槽和在途帧中的最早单调时间作为安全水位，只提交水位之前的结果，避免慢 Lane 的旧结果在窗口已经冻结后到达。候选状态、跨相机合并窗口和冻结决策仍只由 `event-processing` 串行管理。候选确认使用结果单调时间的持续区间；阈值跌落立即重置，合格结果间隔大于两个配置周期从当前结果重计，外部信号只可确认两周期内仍新鲜的合格结果。每相机 `CameraTracker` 另保存不改变 `CandidateEventState` 的 `rearm_pending`、恢复起点/最近正常时间和抑制计数；任一终态建立锁存，只有 cooldown 到期且严格正常结果连续达到 `rearmDurationMs` 才解除，异常或超过两周期的正常结果间隔重置恢复区间。锁存期间自动异常不进入窗口或来源计数，人工结果可绕过锁存，但已有活动 Candidate 时只参与判定。`algorithm.results` 满载时拒绝来源结果、记录 `ALGORITHM_RESULT_QUEUE_FULL` 并立即把来源 Lane 降级为 `manual-trigger-only`；其他 Lane 保持自动检测。

停止顺序固定为：禁止新提交并立即唤醒 worker，每 Lane 最多排空一张人工帧和一张最新自动帧，排空结果入口并冻结剩余窗口，随后关闭内存环、JPEG 和持久化运行时。重配置先构造完整检测器/Lane/两个槽位，并创建停在启动门后的候选事件线程和 Lane worker；只有候选线程组全部准备成功，才有界排空旧线程组。排空后为仍启用且 ID 相同的相机导出重新布防种子：活动 Candidate 经确定性停止进入 Timeout，终态/冷却/已锁存状态均保持锁存，并继承尚未到期的 cooldown；新管线不继承正常累计时长。新启用相机从已布防状态开始，准备失败继续使用旧管线和旧状态。检测调用超过配置周期时累计 `missedProcessingSlots` 并沿用积压报警和五窗口降级；普通节拍抽样和重新布防抑制不进入该报警路径。
...
- 帧扇出先更新每相机预览最新帧槽，再提交事件和 NVMe 分支；单编码线程按固定相机顺序轮转起点，每轮每路最多编码一帧，并由条件变量唤醒；
- 每路分别统计抽样、编码前覆盖、编码、投递和最后投递时间，持续两轮无投递可诊断为单路饥饿；
- 原始高速图像不经预览通道上传。
...
MVS Line 0 上升/下降事件
  → C 回调仅解析原始电平
  → 每相机容量 1 邮箱（覆盖旧边沿、修订递增）
  → 单一可停止 I/O 分发线程
  → 运行时当前状态快照
  → 服务按 High/Low 换算 alarmActive
  → camera.lineInputChanged（每相机独立合并键）
  → Qt CameraClient / 红绿黄灰指示灯
```

回调不构造 JSON、不写盘、不执行网络、编码、推理或 UI 操作。`camera.list`/`camera.getConfig`
只读线程安全快照，是重连和漏推送恢复的事实源。Line 0 状态不进入系统报警注册表或断纸事件状态机。
Line 1 使用设备的 Strobe 节点和 `ExposureStartActive` 源，不用软件线程产生脉冲。
...
- 每路控制事务使用独立运行时互斥；设备描述、能力和最近一次成功连接/更新后的已验证实际参数使用独立短锁缓存。`camera.list` 和采集中的 `camera.getConfig` 只读该缓存及线程安全采集统计，不访问 MVS 节点，也不等待帧池分配或慢控制事务；
- 启动先完成有界帧池和投递资源分配，再开始设备取流；分配失败保持 `Connected`，不留下半启动会话；
...
- 请求处理分为容量 128 的顺序控制通道（1 个工作线程）和容量 512 的只读查询通道（2 个工作线程）；未知命令默认进入控制通道，任一通道满载立即返回 `IPC_BUSY`；
- 两通道共享停止、排空和总截止时间语义，分别公开队列深度、高水位和请求耗时，同时保留汇总指标；
...
当前公开配置为 schema v8；v2～v4 读取时迁移为 `downsampleMode=disabled`、
`processingFps=60`，并把旧连续帧数按 60 FPS 换算后向上取整到 10 ms；v2～v5 均补入
`rearmDurationMs=500`，v2～v7 补入 `timeSync` 默认值并在保存时输出完整 v8。`timeSync` 包含采样周期、
探针超时、接收时钟不确定度、Warning/Alarm 阈值和持续时间，且作为重启生效配置处理，避免运行中替换
探针所有权。启动读取 v7 时先保存原始历史副本，再原子替换主配置；任一步失败均不启用半迁移配置。
新安装默认 `half + 15 FPS + 120 ms` 和 500 ms 重新布防稳定时间。v1～v7 schema 文件继续归档用于
迁移测试；v1 和未来版本拒绝。回滚旧程序前必须从自动配置历史恢复最后一份 v7 文件，不能直接降写
v8；事件、数据库和 NVMe 数据的持久格式未改变，不随配置回滚删除。
...
- Uplink v1 正式使用明文 HTTP/WS 且无应用鉴权，明确接受窃听、伪造命令和中间人风险；网络隔离和端口限制可由现场按需采用，但不属于本项目验收门禁；
- 本项目不规划使用证书、密钥、TLS 或认证的 Uplink 新版本；
...
服务固定线程名包括 `service-main`、`ipc-event`、`ipc-control`、`ipc-query-1`、`ipc-query-2`、每相机的
